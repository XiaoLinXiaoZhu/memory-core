...
新的架构使用类似于 Zettelkasten 卡片盒笔记法，将记忆内容分为多个独立的卡片，每个卡片只包含一个主题或概念。这样可以减少提示词的开销，同时提高模型的灵活性和适应性。
...
我们取消 任何的标签或者元数据，使用“卡片”化的信息来组织记忆。

#### 外链

我们通过 `[[ABC]]` 的方式来引用其他卡片。这些卡片可以是存在的也可以是不存在的。

外联内容可以被 “展开” 成为卡片的内容。展开后，内容会被插入到当前卡片中。形式为：
```md
![[ABC]]start
...
每个卡片都是独立的，内容可以是任意的文本。

#### 对 ai 提供方法

我们不提供任何 内容查询功能，这样能够彻底避免search带来的缺陷，取而代之，我们使用强迫ai自行维护卡片内容的完整性和准确性。

比如可以通过引导 ai 维护一个 [[hint]] 的卡片，让它能够在任何时候都能从中获取有用的相关文件提示。
...
从名称为 `文件名称` 的卡片中获取内容，并根据 `展开深度` 决定展开的层级。
...
将 `文件名称` 的内容设置为 `内容`。如果 `文件名称` 不存在，则会创建一个新的卡片。
...
将名称为 `旧文件名称` 的卡片重命名为 `新文件名称`。如果 `新文件名称` 已经存在，则会将其内容合并到 `旧文件名称` 中。
